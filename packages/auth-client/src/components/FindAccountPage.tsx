...
export interface FindAccountPageProps {
...
export function FindAccountPage({
  ...
  const [activeTab, setActiveTab] = useState<'email' | 'password'>(defaultTab);
  ...
  const brandName = customBrandName || config.brandName || '계정 정보 찾기';
  ...
  return (
    <div className="pt-24 pb-20 px-4 sm:px-6 md:px-12 max-w-[500px] mx-auto min-h-screen bg-[#0a0a0a] flex flex-col justify-center font-sans text-white relative">
      {/* HEADER */}
      <div className="text-center mb-8">
        ...
        <h2 className="text-3xl md:text-4xl font-display font-black tracking-tight text-white uppercase mb-2">
          계정 <span className="text-[#d2ff00]">정보 찾기</span>
        </h2>
        ...
        <div className="grid grid-cols-2 gap-2 p-1 bg-[#0a0a0a] border border-white/5 rounded-xl mb-6 font-mono text-xs">
          <button
            ...
            className={`py-3 rounded-lg font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
              activeTab === 'email'
                ? 'bg-[#d2ff00] text-black font-black shadow-md shadow-[#d2ff00]/10'
          ...
          <div>
            {foundEmailResult ? (
              <div className="text-center py-4 space-y-5">
                ...
                <div>
                  ...
                  <div className="bg-[#0a0a0a] border border-white/10 rounded-xl p-4 my-3 text-center">
                    <span className="text-[#d2ff00] text-base md:text-lg font-mono font-black select-all">
                ...
                <div className="pt-2 space-y-2">
                  {onNavigateLogin && (
                    <button
                      ...
                      className="w-full bg-[#d2ff00] hover:bg-white text-black font-black py-3.5 rounded-xl text-xs uppercase tracking-wider transition-all cursor-pointer"
                    >
                      이메일로 로그인하기
                    </button>
                  )}
              ...
              <form onSubmit={handleFindEmailSubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-[10px] sm:text-xs text-white/50 block font-semibold mb-1.5">성명 <span className="text-[#d2ff00]">*</span></label>
                  <div className="relative">
                    <input
                      ...
                      placeholder="가입 시 입력한 성명"
                      className="w-full bg-[#0a0a0a] border border-white/10 rounded-xl py-3 px-4 text-sm text-white placeholder-white/20 focus:outline-none focus:border-[#d2ff00]/40 transition-colors"
                    />
                ...
                <div>
                  <label className="text-[10px] sm:text-xs text-white/50 block font-semibold mb-1.5">휴대전화 번호 <span className="text-[#d2ff00]">*</span></label>
                  <div className="relative">
                    <input
                      type="tel"
                      value={findTel}
                ...
                <div className="bg-[#2d4a1f]/20 border border-[#d2ff00]/20 rounded-xl p-3 text-[10px] text-white/60 leading-relaxed flex items-center gap-2">
                  <ShieldAlert className="w-3.5 h-3.5 text-[#d2ff00] shrink-0" />
                ...
                <button
                  ...
                  className="w-full bg-[#d2ff00] hover:bg-white text-black font-black py-3.5 rounded-xl text-xs sm:text-sm transition-all shadow-[0_0_15px_rgba(210,255,0,0.15)] flex items-center justify-center gap-2 mt-2 cursor-pointer"
                >
                  {isFindingEmail ? (
                    <>
          ...
          <div>
            {isResetSent ? (
              <div className="text-center py-4 space-y-5">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/70 leading-relaxed break-keep">
                    입력하신 <strong className="text-[#d2ff00] font-mono">{resetEmail}</strong> 주소로 비밀번호 재설정 이메일을 발송했습니다.<br />
                    안내 링크를 클릭하여 새 비밀번호를 설정해 주세요.
                  </p>
                </div>
                {onNavigateLogin && (
                  <button
              ...
              <form onSubmit={handleResetPasswordSubmit} className="space-y-4">
                ...
                <div>
                  <label className="text-[10px] sm:text-xs text-white/50 block font-semibold mb-1.5">가입 이메일 주소 <span className="text-[#d2ff00]">*</span></label>
                  <div className="relative">
                    <input
                      type="email"
                      value={resetEmail}
        ...
        {onNavigateLogin && (
          <div className="pt-6 mt-6 border-t border-white/10 text-center">
            <button
              type="button"
              onClick={onNavigateLogin}
              className="inline-flex items-center gap-1.5 text-xs text-white/50 hover:text-[#d2ff00] font-bold transition-colors cursor-pointer"
            >
