...
import { User as UserIcon, Phone, Calendar, Building, AlertTriangle, Loader2 } from 'lucide-react';
...
export function AdditionalInfoPage({
  onSuccess,
  customBrandName,
  gymLabel = "소속 체육관 (무소속 시 '무소속' 기재)",
}: AdditionalInfoPageProps) {
  const { user, completeAdditionalInfo, isLoading, config } = useAuthStore();
  ...
  const brandName = customBrandName || config.brandName || '회원 필수 정보 기재';
  ...
  useEffect(() => {
    ...
    setValidation({
      ...
      gym: !isGymValid && gymStr.length > 0,
  ...
  return (
    <div className="pt-24 pb-20 px-4 sm:px-6 md:px-12 max-w-[540px] mx-auto min-h-screen bg-[#0a0a0a] flex flex-col justify-center text-white">
      {/* HEADER SECTION */}
      <div className="text-center mb-8">
        ...
        <p className="text-white/60 text-xs md:text-sm font-sans break-keep leading-relaxed px-4">
          서비스 연계를 위한 필수 인적사항(성명, 성별, 생년월일, 연락처, 소속)을 기입하여 가입을 마쳐주십시오.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {user && (
            <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-4 text-xs font-mono">
              ...
              <span className="text-[#d2ff00] ml-2 text-[9px] bg-[#d2ff00]/10 border border-[#d2ff00]/30 px-1.5 py-0.5 rounded uppercase font-semibold">{user.provider}</span>
            </div>
          )}
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full bg-[#0a0a0a] border rounded-xl py-3 px-4 text-sm text-white placeholder-white/20 focus:outline-none transition-colors ${
                    validation.name ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-[#d2ff00]/40'
                  }`}
                />
            ...
            <div className="relative">
              <input
                ...
                className={`w-full bg-[#0a0a0a] border rounded-xl py-3 px-4 text-sm text-white placeholder-white/20 focus:outline-none font-mono tracking-wide transition-colors ${
                  validation.birth ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-[#d2ff00]/40'
                }`}
              />
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className={`w-full bg-[#0a0a0a] border rounded-xl py-3 px-4 text-sm text-white placeholder-white/20 focus:outline-none font-mono tracking-wide transition-colors ${
                  validation.tel ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-[#d2ff00]/40'
                }`}
              />
          ...
          {/* 소속 */}
          <div>
            <label className="text-[10px] sm:text-xs text-white/50 block font-semibold mb-1.5">{gymLabel}</label>
            <div className="relative">
              <input
                type="text"
                value={formData.gym}
                ...
                disabled={isLoading}
                placeholder="소속 단체명 또는 무소속"
                className={`w-full bg-[#0a0a0a] border rounded-xl py-3 px-4 text-sm text-white placeholder-white/20 focus:outline-none transition-colors ${
                  validation.gym ? 'border-red-500/50 focus:border-red-500' : 'border-white/10 focus:border-[#d2ff00]/40'
                }`}
              />
              <Building className="absolute right-3.5 top-3.5 w-4 h-4 text-white/20" />
            </div>
            {validation.gym && <p className="text-red-400 text-[10px] mt-1.5">소속 단체를 기재해 주세요.</p>}
          </div>

          <button
            type="submit"
            disabled={!isValid || isLoading}
            className="w-full bg-[#d2ff00] disabled:bg-white/10 disabled:text-white/20 hover:bg-white text-black font-black py-4 rounded-xl text-xs sm:text-sm transition-all shadow-[0_0_15px_rgba(210,255,0,0.15)] flex items-center justify-center gap-2 duration-200 mt-4 cursor-pointer"
          >
            {isLoading ? (
              <>
